...
import com.sap.appodatav4.databinding.FragmentOrderitemsetCreateBinding;
...
import com.sap.cloud.android.odata.container.ContainerMetadata.EntityTypes;
...
/**
 * A fragment that presents a screen to either create or update an existing OrderItem entity.
 * This fragment is contained in the {@link OrderItemSetActivity}.
 */
public class OrderItemSetCreateFragment extends InterfacedFragment<OrderItem> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OrderItemSetCreateFragment.class);
    ...
    /** OrderItem object and it's copy: the modifications are done on the copied object. */
    private OrderItem orderItemEntity;
    private OrderItem orderItemEntityCopy;

    /** DataBinding generated class */
    private FragmentOrderitemsetCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for OrderItemSet to enter values for the properties.
     ...
     *            OrderItem if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.orderItem.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.orderItem.getLocalName();
        }

        ((OrderItemSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            orderItemEntity = createOrderItem();
        } else {
            orderItemEntity = viewModel.getSelectedEntity().getValue();
        }

        OrderItem workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (OrderItem)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            orderItemEntityCopy = (OrderItem) orderItemEntity.copy();
            orderItemEntityCopy.setEntityTag(orderItemEntity.getEntityTag());
            orderItemEntityCopy.setOldEntity(orderItemEntity);
            orderItemEntityCopy.setEditLink((orderItemEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            orderItemEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, orderItemEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((OrderItemSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(orderItemEntityCopy);
        } else {
            viewModel.update(orderItemEntityCopy);
        }
        return true;
    }

    /**
     * Create a new OrderItem instance and initialize properties to its default values
     * Nullable property will remain null
     * For offline, keys will be unset to avoid collision should more than one is created locally
     * @return new OrderItem instance
     */
    private OrderItem createOrderItem() {
        OrderItem orderItemEntity = new OrderItem(true);
        orderItemEntity.unsetDataValue(OrderItem.orderNumber);
        orderItemEntity.unsetDataValue(OrderItem.position);
        return orderItemEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<OrderItem> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(orderItemEntityCopy);
            }
            if( isMasterDetail ) {
                OrderItemSetListFragment listFragment = (OrderItemSetListFragment) currentActivity.getSupportFragmentManager().findFragmentByTag(UIConstants.LIST_FRAGMENT_TAG);
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentOrderitemsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setOrderItem(orderItemEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isOrderItemValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_orderitem);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.orderItem.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<OrderItem> result) {
        String errorMessage;
