...
import com.sap.appodatav4.archcomp.SingleLiveEvent;

import com.sap.appodatav4.repository.OperationResult.Operation;
...
public class Repository<T extends EntityValue> {
     ...
     * @param container - OData service
     * @param entitySet - the entity set this repository provides access for
     */
    public Repository(@NonNull Container container, @NonNull EntitySet entitySet, @Nullable Property orderByProperty) {
    ...
    public void read() {
        ...
        container.executeQueryAsync(dataQuery,
            result -> {
    ...
    public void read(EntityValue parent, String navigationPropertyName) {
        ...
        container.loadPropertyAsync(navigationProperty, parent, dataQuery,
            () -> {
    ...
    public void create(@NonNull T newEntity, @NonNull StreamBase media) {
        if (newEntity.getEntityType().isMedia()) {
            container.createMediaAsync(newEntity, media,
                () -> {
    ...
    public void create(@NonNull T newEntity) {
        ...
        container.createEntityAsync(newEntity,
            () -> {
                insertToCache(newEntity, entities);
                if (relatedEntities.size() > 0) {
                    insertToCache(newEntity, relatedEntities);
    ...
    public void update(@NonNull T updateEntity) {
        container.updateEntityAsync(updateEntity,
            () -> {
    ...
    public void delete(@NonNull List<T> deleteEntities) {
        ...
        container.applyChangesAsync(deleteChangeSet,
            () -> {
     ...
     */
    public void downloadMedia(@NonNull T entity, Action1<byte[]> successHandler,
            Action1<RuntimeException> failureHandler) {
        container.downloadMediaAsync(entity,
    ...
    private void replaceInCache(@NonNull T updateEntity, List<T> cache) {
        int index = 0;
        for (T entity: cache) {
                if (entity.getReadLink().equals(updateEntity.getReadLink())) {
                    if (orderByProperty != null) {
                        cache.remove(index);
    ...
    private void removeFromCache(@NonNull T deleteEntity, List<T> cache) {
        int index = 0;
        for (T entity: cache) {
                if (entity.getReadLink().equals(deleteEntity.getReadLink())) {
                cache.remove(index);
                break;
            }
            index++;
